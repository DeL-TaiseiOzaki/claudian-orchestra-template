...
> - **Vault のリファレンスノート作成は Claude の責務**（呼び出し元）。Claude は `hermes chat -q -s genspark-slide` で本スキルを叩き、返ってきた `task_id` / URL / PPTX path を `Wiki/{topic}.md` 等に手で書く。
