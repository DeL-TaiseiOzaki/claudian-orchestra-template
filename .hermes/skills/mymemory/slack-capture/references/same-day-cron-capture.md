...
- For existing same-day digests, prefer the existing file under `Inbox/{YYYY-MM-DD}/slack/`, repair frontmatter to the current Output spec, and append only missing message entries keyed by `channel_id + ts`. Preserve existing body content, attachments, thread context, and permalink text.
