...
> **read 専用**です。Drive 側の正本はそのまま(system of record は Drive)、vault には読んだ内容の**蒸留ノート**(`{area}/sources/` へのポインタ md や `docs/` の要約)だけを残します。vault から Drive への書き込み・同期はしません。
...
- 読んだ内容の蒸留は通常フロー(`Work/{X}/docs/` や `references/` に Claude がノート化、元ファイルへのリンクを `resource:` frontmatter で保持)
