...
- どちらも「clippings 集約やって」「chat-logs 集約やって」で Daily に集約 → EOD で `Others/Ideas/` や各プロジェクトの `sources/` へ蒸留
...
- **拡張自体はこのテンプレートに含まれていません**(各自実装)。ペイロード契約(JSON スキーマ)は [[.hermes/skills/mymemory/clippings-capture/SKILL.md]] に定義されています
- 本文抽出が必要な場合は Hermes 側の `defuddle` skill が URL → Markdown 変換を担います
- 自動化度は最も高いですが、まず経路 A で運用を回してから移行するのがおすすめです

## 4. 経路 C:手動

拡張なしでも成立します。テキストをコピーして `Inbox/{date}/clippings/{適当なslug}.md` に貼り付けるだけです。Claude Code に「これ clippings に入れておいて」と貼り付けて頼んでも構いません(保存先が自明なものは直接配置し、Daily に記載が残ります)。
...
- [[.hermes/skills/mymemory/clippings-capture/SKILL.md]] — webhook 経路のペイロード契約
- [[.hermes/skills/mymemory/defuddle/SKILL.md]] — URL → Markdown 本文抽出
