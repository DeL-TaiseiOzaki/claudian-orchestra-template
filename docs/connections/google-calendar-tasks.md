...
2. `.hermes/skills/mymemory/inbox-daily-capture/scripts/calendars.local.json` を作成(gitignore 済み):
...
3. 動作確認:
   ...
   ```bash
   uv run .hermes/skills/mymemory/inbox-daily-capture/scripts/fetch_calendar_ics.py --format md
   ```
...
6. **認可(この vault の tracked スクリプトで)**:bundled の setup.py ではなく、**必要スコープ全部を 1 回の同意で取る** `google-auth` skill を使います
   ...
   python .hermes/skills/mymemory/google-auth/scripts/authorize.py --auth-code "<リダイレクト URL 全体>"
...
- [[.hermes/skills/mymemory/google-auth/SKILL.md]] — OAuth スコープ設計・再認可・スコープ追加の方法
- [[.hermes/skills/mymemory/inbox-daily-capture/SKILL.md]] — 朝 capture 本体(ics / gws の使い分け・フォールバック)
- [[.hermes/skills/mymemory/google-tasks/SKILL.md]] — Tasks 取得スクリプト
- 複数 Google アカウント(会社 Workspace など)を扱いたい場合 → [[.hermes/skills/mymemory/google-auth/references/gws-cli-calendar-tasks.md]](gws CLI の config dir 切り替え)
