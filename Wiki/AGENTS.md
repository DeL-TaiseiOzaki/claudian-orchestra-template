...
人と AI が同じ Markdown を読み書きする **general wiki**（Karpathy の "LLM wiki" 思想）。詳細ルールは [[.codex/rules/wiki-management.md]]。
...
├── sources/        # ① Raw：Inbox 由来 raw の archive（非 md 可・immutable・type: capture / status: inbox の path override）
└── _assets/        # 非 md 作業物
```
...
- compiled note の frontmatter は標準 enum（[[.codex/rules/vault-metadata.md]]）。`Wiki/sources/` だけは raw-capture path override を使う。compiled note は `type` で分類する：
...
- 流入：EOD distill が Daily から蒸留（[[.codex/skills/eod-distill/SKILL.md]]）、または保存先が自明なら直接配置（Daily に記載を残す）
- 破棄：消さず `status: archived`（必要なら `Archive/` へ退避）

## やってはいけない

- 生ログを直置きしない（Inbox → Daily 集約 → distill の導線を通す。例外は保存先自明時の直接配置のみ）
- エージェント運用の学び（gotcha・root cause）は `.codex/docs/knowledges/` へ（Wiki は人間の知識、knowledges はエージェントの運用知）
