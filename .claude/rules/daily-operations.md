...
| 3 | 作業 | 日中 continuous | event | User+コアエージェント | Work logs／Daily 追記／`Inbox/{YYYY-MM-DD}/clippings/` 等。skills：`work-project-writer`／`others-writer` |
...
- `Work/{PROJ_A|PROJ_B|PROJ_C}/logs/{YYYY-MM-DD}.md`（触った案件のみ作成）
...
Gmail は briefing に入れない（on-demand pull のみ・[[docs/connections/gmail.md]]）。
...
Weekly は触った案件をすべて入れる（`PROJ_A` / `PROJ_B` / `PROJ_C`）。
