...
Main DB は **Wiki 一本**（エージェント運用知のみ `.claude/docs/knowledges/`）。
...
frontmatter スキーマの単一の正は [[.claude/rules/vault-metadata.md]]、タグ体系は [[.claude/rules/vault-tagging.md]]。

## 配置規則

- **フラット基本**：`Wiki/{slug}.md`（1 ノート = 1 トピック）。同一テーマのノートが 3 枚を超えたら `Wiki/{topic}/` にサブフォルダ化してよい（移動時はリンクを保つ）
- 会議メモ：`Wiki/meetings/{YYYY-MM-DD}-{topic}.md`（AI 議事録の要約着地先。テンプレ [[Templates/meeting-note.md]]。[[.claude/skills/mtg-prep/SKILL.md]] / [[.claude/skills/aggregate-mtgs/SKILL.md]]）
- Inbox 由来の raw（clippings / chat-logs / discord digest 等）を残す場合：`Wiki/sources/` に raw のまま（`type: capture` 保持・immutable）。蒸留ノートは Wiki 直下に別途書き、raw へ相対リンク
...
- 破棄するノートは削除せず `status: archived`（肥大化したら [[.claude/skills/vault-archive/SKILL.md]] で `Archive/` へ）
- 定期棚卸し：weekly-review 時に `draft` のまま放置された idea を昇格 / archived に振り分ける

## やってはいけない

- 生ログの直置き（→ `Inbox/` 経由。保存先自明の直接配置時も Daily に記載）
- エージェント運用の学び（gotcha / root cause / 外部 CLI の仕様変化）を Wiki に書かない（→ `.claude/docs/knowledges/`、[[.claude/skills/knowledge-capture/SKILL.md]]）
- 自己紹介・経歴を書かない（→ `Persona/`）

## 関連

- [[Wiki/AGENTS.md]] / [[.claude/skills/wiki-writer/SKILL.md]]
- [[.claude/rules/vault-metadata.md]] / [[.claude/rules/vault-tagging.md]] / [[.claude/rules/daily-operations.md]]
- [[.claude/skills/eod-distill/SKILL.md]]（Daily → Wiki の蒸留）
