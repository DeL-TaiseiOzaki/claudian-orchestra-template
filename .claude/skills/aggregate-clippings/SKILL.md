...
description: Read Hermes-staged web clippings at Inbox/{YYYY-MM-DD}/clippings/*.md and aggregate concise reference bullets (title / 1-line summary / tag hints) into today's root Daily note (Daily/{YYYY-MM-DD}.md). Routes to 📚 Wiki. Append-only with wikilinks back to the raw clipping. Use on-demand when the Chrome extension or `clippings-capture` has landed new files mid-day; EOD distill later moves durable ones to Wiki/sources/ or Wiki/.
---
