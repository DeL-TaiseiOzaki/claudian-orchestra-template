...
|---|---|
| `[PROJ_A]` / `[PROJ_B]` / `[PROJ_X]` / `[PROJ_D]` / `[PROJ_E]` / `[PROJ_C]` / `[PROJ_F]` prefix | `### 🏢 Work` の該当案件 bullet 配下 |
| 学会・コミュニティ（YSAI / YANS / Kaggle 等） | `### 💡 Others / Insights`（Activities） |
...
- [[.hermes/skills/mymemory/genspark-mtg/SKILL.md]]（capture 側）
