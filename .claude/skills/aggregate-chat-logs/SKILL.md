...
description: Read AI chat exports (ChatGPT / Claude conversation transcripts) at Inbox/{YYYY-MM-DD}/chat-logs/{provider}-{slug}.md and aggregate concise discussion bullets (theme / main conclusions / open questions) into today's root Daily note (Daily/{YYYY-MM-DD}.md). Routes to 📚 Wiki. Append-only with wikilinks back to the raw chat. Use on-demand when a new AI Exporter clipping has landed mid-day; EOD distill later moves durable insights to Wiki/ or .claude/docs/knowledges/.
---
...
> **位置づけ**：会話ログは「壁打ち・思考の足跡」。生の対話は重いので Daily にはサマリだけ。EOD distill で durable な insight だけ `Wiki/` ／ `.claude/docs/knowledges/` に蒸留する。
...
- **EOD distill で配分**：durable な insight は `Wiki/` ／ `.claude/docs/knowledges/` へ
...
- [[.claude/skills/wiki-writer/SKILL.md]]（Wiki ノート作成）
