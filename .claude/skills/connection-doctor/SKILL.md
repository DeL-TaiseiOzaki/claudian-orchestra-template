...
| **Google** | `~/.hermes/google_token.json` の存在 | `python .hermes/skills/mymemory/google-auth/scripts/authorize.py --check`(AUTHENTICATED + スコープ一覧)。ics 経路は `calendars.local.json` の存在 + `fetch_calendar_ics.py --format md` |
