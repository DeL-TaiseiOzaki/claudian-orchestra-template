...
`hermes chat -q` が非対話ワンショット。**PowerShell ツール**で実行する（hermes.exe は PowerShell の PATH にあるが Bash ツールの PATH には無い）。

```powershell
# Windows（日本語ロケール）必須：cp932 デコード死を防ぎ UTF-8 で取得する
$env:PYTHONUTF8 = '1'
...
**タイムアウト**：LLM セッション + 外部 API 呼び出しで 30–120 秒かかる。PowerShell ツールの `timeout` を `180000`（3 分）程度に。

**⚠️ 文字コード（日本語 Windows 必須）**：`hermes`（Python）は子プロセス出力を **cp932** でデコードするため、`’`（U+2019, byte 0x92）や日本語が混じると `UnicodeDecodeError` でリーダースレッドが死に、**rc=0 なのに stdout が空 / 欠落**する（エラーにならず気づきにくい）。上記のとおり呼び出し前に **`$env:PYTHONUTF8 = '1'`** を必ず設定（UTF-8 モード強制）。恒久化は User 環境変数 `PYTHONUTF8=1`。詳細 → [[.claude/docs/knowledges/python/windows-cp932-stdout-default.md]]。

**Bash ツールから呼ぶ場合**のフォールバック（**ユーザ名をハードコードしない**。PC により `user-a` / `user-b` 等と異なる）：
```bash
# PATH に通った hermes を使うのが第一。フルパスが要るなら $USERNAME で組む
PYTHONUTF8=1 "/c/Users/$USERNAME/AppData/Local/hermes/hermes-agent/venv/Scripts/hermes.exe" chat -q "<指示>" -Q --source claude-code
```
...
- **失敗時**（未認証・接続不可・タイムアウト・**出力空**）：hermes の stderr/末尾メッセージを 1 行で要約し、原因（auth 失効？ **rc=0 で出力空なら cp932** → `$env:PYTHONUTF8=1`）と次アクションを提示。`hermes doctor` / `hermes status` で切り分け可。※`chat -q` は単発スタンドアロンなので gateway 稼働は不要（gateway 停止は pull 失敗の原因ではない）。
...
- [[.hermes/skills/autonomous-ai-agents/hermes-agent/SKILL.md]] — hermes CLI リファレンス（`hermes chat -q` 他）
- [[.claude/docs/knowledges/python/windows-cp932-stdout-default.md]] / [[.claude/docs/knowledges/hermes/multi-pc-pull-cli-setup.md]] — Windows cp932 落とし穴・多PC pull セットアップ
