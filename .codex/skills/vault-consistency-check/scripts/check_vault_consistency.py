...
SCHEMA_RULES_PATH = Path(
    ".codex/skills/vault-consistency-check/references/schema_rules.json"
)
SCHEMA_REQUIRED_KEYS = frozenset(
    {
        "common_required",
        "type_enum",
        "status_enum",
        ...
        "experiment_required",
        "raw_capture_paths",
        "raw_capture_type_enum",
        "raw_capture_status_enum",
        "raw_capture_common_required",
        "knowledge_note_paths",
        "knowledge_common_required",
        "knowledge_type_enum",
        "knowledge_status_enum",
        "knowledge_severity_enum",
        "knowledge_superseded_required",
        "knowledge_deprecated_required",
...
def scannable_note_path(rel_path: str) -> bool:
    """Return whether a path is an in-scope note or template for scans."""
    ...
    if parts[-1] in SCAN_EXCLUDED_FILENAMES:
        return False
    if rel_path.startswith(".codex/docs/knowledges/"):
        return not any(part.startswith(".") for part in parts[3:])
...
def read_text(path: Path) -> str:
    """Read UTF-8 text while accepting an optional BOM."""

    return path.read_text(encoding="utf-8-sig")


def load_schema_rules(vault_root: Path) -> dict[str, Any]:
    """Load the dependency-free JSON schema single source of truth."""

    path = vault_root / SCHEMA_RULES_PATH
    try:
        data = json.loads(path.read_text(encoding="utf-8-sig"))
    except FileNotFoundError as error:
        raise RuntimeError(f"schema rules file is missing: {SCHEMA_RULES_PATH}") from error
    except json.JSONDecodeError as error:
        raise RuntimeError(f"schema rules JSON is invalid: {error}") from error
    if not isinstance(data, dict):
        raise RuntimeError("schema rules JSON root must be an object")
    missing = sorted(SCHEMA_REQUIRED_KEYS - data.keys())
    if missing:
        raise RuntimeError(f"schema rules JSON is missing keys: {', '.join(missing)}")
    return data
...
def parse_minimal_yaml(block: str) -> dict[str, Any]:
    """Parse the scalar and list subset used by vault frontmatter."""
    ...
    def inline_list(value: str) -> list[Any]:
        items: list[str] = []
        current: list[str] = []
        in_single = False
        in_double = False
    ...
    for raw_line in block.splitlines():
        ...
        if line.startswith("  - ") and current_key is not None and current_list is not None:
            current_list.append(scalar(line[4:].strip()))
            continue
        current_key = None
        current_list = None
        if ":" not in line:
...
def load_frontmatter_and_body(path: Path) -> tuple[dict[str, Any], str]:
    """Load leading frontmatter deterministically with the stdlib parser."""
...
def strip_markdown_code(text: str) -> str:
    """Remove fenced and inline code before interpreting wikilinks."""
...
def is_raw_capture(rel_path: str, rules: dict[str, Any]) -> bool:
    """Return whether a note uses the immutable raw-capture schema."""

    return path_matches_any(rel_path, rules["raw_capture_paths"])


def is_knowledge_note(rel_path: str, rules: dict[str, Any]) -> bool:
    """Return whether a note uses the agent-knowledge schema."""

    return path_matches_any(rel_path, rules["knowledge_note_paths"])
...
def validate_frontmatter(rel_path: str, meta: dict[str, Any], rules: dict[str, Any]) -> list[str]:
    ...
    errors: list[str] = []
    if is_raw_capture(rel_path, rules):
        for key in rules["raw_capture_common_required"]:
            if key not in meta:
                errors.append(f"required field missing: {key}")
        if meta.get("type") not in rules["raw_capture_type_enum"]:
            errors.append(f"invalid raw capture type: {meta.get('type')}")
        if meta.get("status") not in rules["raw_capture_status_enum"]:
            errors.append(f"invalid raw capture status: {meta.get('status')}")
        return errors
    if is_knowledge_note(rel_path, rules):
        for key in rules["knowledge_common_required"]:
            if key not in meta:
                errors.append(f"knowledge field missing: {key}")
        if meta.get("type") not in rules["knowledge_type_enum"]:
            errors.append(f"invalid knowledge type: {meta.get('type')}")
        if meta.get("status") not in rules["knowledge_status_enum"]:
            errors.append(f"invalid knowledge status: {meta.get('status')}")
        if meta.get("severity") not in rules["knowledge_severity_enum"]:
            errors.append(f"invalid knowledge severity: {meta.get('severity')}")
        if meta.get("status") == "superseded":
            for key in rules["knowledge_superseded_required"]:
                if key not in meta:
                    errors.append(f"superseded knowledge field missing: {key}")
        if meta.get("status") == "deprecated":
            for key in rules["knowledge_deprecated_required"]:
                if key not in meta:
                    errors.append(f"deprecated knowledge field missing: {key}")
        return errors
        ...
        findings.append(
            Finding(
                ...
                finding_message(
                    ...
                    "想定構造なら schema_rules.json の structure_expected_root_dirs に登録、不要なら整理してください",
                ),
            )
        )
            ...
            findings.append(
                Finding(
                    ...
                    finding_message(
                        ...
                        ".gitkeep 付きで作成するか、不要なら schema_rules.json の structure_required_dirs から外してください",
                    ),
                )
            )
                ...
                if sub.is_file():
                    ...
                    findings.append(
                        Finding(
                            ...
                            sub_rel,
                            finding_message(
                                f"`{sub_rel}` は日付フォルダ直下の file です。",
                                "`Inbox/YYYY-MM-DD/{source}/` の既知 source サブフォルダへ capture し直してください",
                            ),
                        )
                    )
                    continue
                ...
                findings.append(
                    Finding(
                        ...
                        finding_message(
                            ...
                            "Inbox/{YYYY-MM-DD}/attachments/ か対象フォルダ直下の _assets/ に移すか、コード本体は GitHub を正本にしてください",
                        ),
                    )
                )
...
def render_markdown(
    findings: list[Finding], mode: str, ran_at: str, checked_count: int = CHECKED_COUNT
) -> str:
...
def main() -> int:
    """Run the CLI, returning nonzero only when the checker itself cannot run."""
    ...
    try:
        ...
        fallback = [Finding("ERROR", "Consistency checker", None, finding_message(f"検査を開始できませんでした: {exc}", "schema とスクリプト実装を確認してください"))]
        output = render_json(fallback, args.mode, ran_at, checked_count=0) if args.as_json else render_markdown(fallback, args.mode, ran_at, checked_count=0)
        sys.stdout.buffer.write(output.encode("utf-8"))
        if not output.endswith("\n"):
            sys.stdout.buffer.write(b"\n")
        return 2
