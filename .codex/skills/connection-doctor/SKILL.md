...
description: Diagnose enabled external connections with read-only checks and report status plus next actions. Use for 「接続チェックして」 or connection failures.
---
...
- **registry を尊重する**:最初に [[.codex/connections.yaml]] を読む。`disabled` の接続は**チェックしない**(未設定はその人の選択)。`unconfigured` は ➖ で報告し、[[.codex/skills/connection-setup/SKILL.md]](「接続セットアップして」)を案内する。全接続が `unconfigured` なら、個別診断より先にセットアップウィザードを提案する
- **read-only**:診断のみ。設定変更・トークン再発行・ファイル削除は**しない**(修復は提案として提示し、対応する [[Meta/connections/README.md]] のガイドへ誘導する)
...
`.codex/connections.yaml` を読み、`enabled` の接続だけを §1 の診断対象にする(`disabled` はスキップ、`unconfigured` は ➖ 行として表にだけ載せる)。
...
| 0-3 | Hermes CLI | `hermes --version` | 無ければ**以降の接続チェックは全部スキップ**し、[[GETTING-STARTED.md]] Level 1 へ誘導 |
| 0-4 | Hermes 応答 | `hermes chat -q "reply with exactly: PONG" -Q`(timeout 180s) | 応答があるか。LLM バックエンド設定の生死がここでわかる |

> Windows 日本語環境では `PYTHONUTF8=1` を設定してから hermes を呼ぶ(cp932 問題 → [[.codex/skills/hermes-query/SKILL.md]])。
...
| **Google Calendar / Tasks** | `~/.hermes/google_token.json` の存在 | [[.hermes/skills/vault-capture/google-auth/SKILL.md]] の手順で Hermes runtime Python を `HERMES_RUNTIME_PY` に解決し、`$HERMES_RUNTIME_PY .hermes/skills/vault-capture/google-auth/scripts/authorize.py --check`（AUTHENTICATED + scope 一覧）。ics 経路は `calendars.local.json` の存在 + `fetch_calendar_ics.py --format md` |
| **Gmail** | 上記 `--check` のスコープ一覧に `gmail.*` が含まれるか | `hermes chat -q "Gmail の未読メール件数だけ教えて" -Q` |
| **Google Drive** | コネクタ経路は コア側の Drive ツールが使えるか(ツール有無で判定)。Hermes 経路はスコープ一覧に `drive` | コネクタ:Drive 検索を 1 回実行 / Hermes:`hermes chat -q "Drive で適当なファイルを 1 件検索してタイトルだけ" -Q` |
...
| **AI 議事録** | Genspark アダプタ利用時のみ:`gsk` が PATH にあるか(`command -v gsk`)。capture extension 経路なら直近の `Inbox/*/mtgs/` の有無 | Genspark:`gsk meeting list`(一覧 or 空リストが返れば OK) |
...
| Slack | ⚠️ | 2026-07-14 | SLACK_USER_TOKEN を追加 → Meta/connections/slack.md §3.2 |
| Genspark | ➖ 未設定 | — | 使うなら Meta/connections/genspark.md |
| Notion | ➖ 未設定 | — | 使うなら Meta/connections/notion.md |
```
...
- [[.codex/connections.yaml]] — 診断対象を決める registry
- [[.codex/skills/connection-setup/SKILL.md]] — 対話式セットアップ(unconfigured の誘導先)
- [[GETTING-STARTED.md]] — 段階的セットアップ(Level 0–2)
- [[Meta/connections/README.md]] — 接続別ガイド(修復手順の誘導先)
- [[.codex/skills/hermes-query/SKILL.md]] — pull 経路の呼び出し規約
- [[.codex/rules/agent-boundaries.md]] §6 — 接続の所有マップ
