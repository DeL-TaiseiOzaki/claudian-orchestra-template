...
### Step 3: 未集約チェック
- repo ごとに Daily 全文で完全な anchor link `[[Inbox/{date}/code/code.md#{owner}-{repo}]]` を検索
- 完全一致があればその repo だけ skip。未一致の repo は追加する
...
`Inbox/{date}/code/code.md` は **単一ファイル** なので、再 aggregate 時は：
- 既に Daily にあった repo の wikilink は skip
- 新規 repo（前回の wikilink リストに無いもの）だけ append
- 増分判定は完全な anchor link の存在チェックを使う
...
- 大きな PR / 設計変更は別途 `Wiki/` や `.codex/docs/knowledges/` へ別ノート（EOD distill / knowledge-capture 経由）

## 他 skill との連携

- [[.codex/skills/eod-distill/SKILL.md]]（per-repo digest を `Wiki/sources/` へ配分）
- [[.hermes/skills/vault-capture/github-eod-capture/SKILL.md]]（capture 側）
- [[.codex/skills/knowledge-capture/SKILL.md]]（コードから得た学び）

## 関連

- [[Maps/Code-Map.md]]（repo インデックス）
- [[.codex/rules/inbox-routing.md]] §3 配分表
- [[.codex/rules/wiki-management.md]]（コード読解ノートのルール）
