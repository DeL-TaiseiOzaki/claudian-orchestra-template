...
> Operating skill: [[.codex/skills/knowledge-capture/SKILL.md]]
> Note format: [[.codex/skills/knowledge-capture/references/template.md]]
> Category definitions: [[.codex/skills/knowledge-capture/references/categories.md]]
> Frontmatter schema: [[.codex/rules/vault-metadata.md]] §5。category note は `type: knowledge` と `status: active | superseded | deprecated` の path override を使い、この README は MOC として標準 `type: note` を使う。
...
1. Trigger `/knowledge-capture` or ask the core agent "これ knowledge 化して" ("capture this as a knowledge note").
...
- Raw investigation results → `.codex/docs/research/`
- A learning that only matters to one skill → that skill's `SKILL.md` `## Pitfalls` section

## See also

- [[.codex/skills/knowledge-capture/SKILL.md]] — operating skill
