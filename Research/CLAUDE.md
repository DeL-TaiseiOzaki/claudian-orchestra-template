# CLAUDE.md — Research（オプション・空のプレースホルダ）
...
サブモジュール側に自前の `CLAUDE.md` / `.claude/rules/` / `status.md` を置き、研究作業のルールはサブモジュール側が「正」になる。vault 側（本ファイル）はそれに優先しない。
...
├── CLAUDE.md       # ② Schema：研究エージェント契約（本ファイルを上書き）
...
- [[Persona/CLAUDE.md]] — 著者プロフィールは vault root から参照
